...
<button class="layui-btn" onclick="Review.auditing_arts()"><i class="layui-icon" style="">&#xe605;</i>批量审核</button>
<button class="layui-btn layui-btn-danger" onclick="Review.delete_reviewes()"><i class="layui-icon" style="">&#xe640;</i>批量删除</button>
</div>
<table class="layui-table layui-form" id="reply">
    <colgroup>
    <col width="30">
    </colgroup>
  	<thead>
  	...
  	<tbody>
        <?php if(is_array($reply)): foreach($reply as $key=>$re): ?><tr data-id="<?php echo ($re["id"]); ?>">
            <td><input type="checkbox" lay-skin="primary" name="select" value="<?php echo ($re["id"]); ?>,<?php echo ($re["article_id"]); ?>"></td>
            ...
            <td><?php echo (htmlspecialchars_decode($re["content"])); ?></td>
            <td>
                <a class="layui-btn layui-btn-danger layui-btn-small delete_art" onclick="Review.delete_review('<?php echo ($re["id"]); ?>','<?php echo ($re["article_id"]); ?>')"><i class="layui-icon" style="">&#xe640;</i></a>
            </td>
    ...
    <script type="text/javascript">
        $(function(){
            Base.initialize();
            Base.urls = {
    review:"<?php echo U('Admin/review/index');?>",
    deleteReview:"<?php echo U('Admin/review/deleteReview');?>",
};
