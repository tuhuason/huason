var Review = {
	initialize : function(settings) {
        layui.use(['form','element','layedit'], function(){
            ...
            form.on('checkbox(review)', function(data){
                //是否被选中，true或者false
                var child = $('#review').find('tbody input[type="checkbox"][name="select"]');
            ...
            form.on('switch(auditing)', function(data){
                ...
                if(checkbox.checked){
                    Review.auditing_review(dataId, 1);
                }else{
                    Review.auditing_review(dataId, 0);
                }
            });
        });
    },
    ...
    //删除评论
    delete_review : function(id, article_id){
    	var datas = {id:id,article_id:article_id};
    ...
    delete_reviewes : function(){
        var datas = [];
        $('#review input:checkbox[name="select"]:checked').each(function(index,dom){
            var val = $(dom).val().split(',');
            datas.push({id:val[0],article_id:val[1]});
        });
